...
void SpriteBatch::Draw( SmartHandle<Texture2D> texture, DemoRectangle destRect, DemoColour col )
{
	sprites.push_back(Sprite(texture, destRect, col));
}

void SpriteBatch::Draw( SmartHandle<Texture2D> texture, float* position, DemoRectangle* srcRect, DemoColour col, float angle, float* origin, float* scale )
{

...
Demo::Demo(HDC deviceContext) : Game(deviceContext), mPerPix(0.0651f), pixPerM(15.36f)//, info(deviceContext)
{
	...
	toDestroy = std::vector<GameObject*>();
	setVSync(0);
}
...
Demo::~Demo(void)
{
	...
	delete crateTex;
	delete boundaryTex;
	delete spawnButtonReleasedTex;
	delete spawnButtonPressedTex;
	delete destroyButtonReleasedTex;
	delete destroyButtonPressedTex;
	delete[] objects;
	for (int i = 0; i < 2; ++i)
	{
		delete buttons[i];
...
void Demo::Initialize()
{
	...
	//Button creation
	buttons[0] = new PushButton(DemoRectangle(0, 0, 130, 75), NULL, DemoColour(1.0f, 1.0f, 1.0f, 1.0f));
	buttons[1] = new ToggleButton(DemoRectangle(0, 150, 130, 75), NULL, DemoColour(1.0f, 1.0f, 1.0f, 1.0f));
...
void Demo::Update( double elapsedTime )
{
	...
	{
		physWorld->DestroyBody(toDestroy[i]->physBody);
		objects[toDestroy[i]->index] = NULL;
		delete toDestroy[i];
	}
	toDestroy.clear();
...
bool Demo::AddGameObject( GameObject* obj )
{
	for (int i = 0; i < maxObjects; ++i)
	{
		if (objects[i] == NULL)
